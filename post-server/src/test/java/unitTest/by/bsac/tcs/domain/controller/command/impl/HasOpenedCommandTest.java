...
public class HasOpenedCommandTest extends GenericCommandTest {

  private static final String EXCEPTED_MESSAGE = "WITHDRAWN_REGISTERED";

  @InjectMocks
  private HasOpenedCommand command;

  @Override
  protected void beforeTestExecuteSuccess() throws ServiceException {
    super.beforeTestExecuteSuccess();
    doNothing().when(eventService).opened(any(EventLog.class));
    when(request.getMethod()).thenReturn(Method.WITHDRAWN);
  }
  ...
  protected void afterTestExecuteSuccess() throws ServiceException {
    ...
    verify(eventService).opened(eventLog);
  ...
  protected void beforeTestExecuteFailsWhenServiceException() throws ServiceException {
    ...
    doThrow(ServiceException.class).when(eventService).opened(any(EventLog.class));
  }
